...
//函数声明部分的接口函数
void Tr_procEntryExit(Tr_level _level, Tr_exp _body, Tr_accessList _formals, Temp_label _label) {
	T_stm stm;
	stm = T_Seq(T_Label(_label), T_Move(T_Temp(F_RV()), unEx(_body)));
	F_Proc(stm, _level->frame);
}
F_fragList Tr_getResult() { return F_getFragList(); }
//获取最终的结果
